...
; Begin Monitoring Drop Folder
bareRepos := hdrive . hpath . "\Bare-Repos"
WatchFolder(bareRepos, "ChangeResponseFunction", True)

ChangeResponseFunction(watchedFolder, Changes){
    ...
    For Each, child in myarray 
    {
        ; I have no idea what I'm doing. I can't work out how to print
        ; the contents of an array. I'm going back to Python
        MsgBox, %child%
        MsgBox, array loop
    }
}
...
^!p::
PrintChangedChildren()
Return

PrintChangedChildren(){
    For Each, child in changedChildren {
        MsgBox, %child%
    }
}

; When changes are reported, get the highest paths below the drop folder. Put paths in export list
;;;;;
; changedChildren := Array()
; for change in Changes
;  pathToChange := change.Name
;  rootChild = GetRootChild()
; if rootChild not in changedChildren
;  changedChildren.append(rootChild)

; On '!^s' pop path from changedChildren and send to VDI
; if changedChildren is empty, print message to user
;;;;

; Make sure each path only exists once in list of exports
